...
spaced repetition по алгоритму SM-2, push-уведомления через бота, система достижений и лидерборд.
...
### 4. Spaced repetition — фиксированные интервалы

Вопросы из квизов попадают в таблицу `review_schedule` **только после ответа**
(не при открытии урока). Состояние каждой карточки:

- `intervalDays` — через сколько дней показать снова
- `nextReviewAt` — конкретная дата следующего показа
- `lastReviewedAt` — когда последний раз отвечал

После ответа вызывается чистая функция `nextSRState(prev, grade, now)`:

```
Правильно (grade ≥ 3): следующий показ через 10 дней
Ошибка    (grade < 3): следующий показ через 5 дней
```

Фиксированные интервалы выбраны осознанно: SM-2 адаптирует интервалы под
историю ответов, но для этого нужны 5–10 ответов на карточку. В учебном
квизе (1–3 попытки на тест) алгоритм не успевает обучиться — поэтому
простая схема честнее и понятнее пользователю.

Чтение вопросов «к повтору сейчас» — индекс
`@@index([userId, nextReviewAt])`, запрос `WHERE next_review_at <= NOW()`
работает за O(log n).

Код: [`lib/sr.ts`](lib/sr.ts), [`app/api/reviews/`](app/api/reviews/), [`app/review/`](app/review/).
...
| Фиксированные интервалы (10/5 дней) | SM-2 / FSRS | SM-2 и FSRS адаптируют интервалы под историю ответов, но для этого нужны 5–10 ответов на карточку. Учебный квиз проходят 1–3 раза — адаптация не успевает работать |
...
- **Тесты на код** (написаны vitest-тесты для `lib/notifications.ts`; `lib/sr.ts` и `lib/datetime.ts` — в очереди)
